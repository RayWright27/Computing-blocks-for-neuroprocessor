...
void conv::convolution(void) {
	//while(true){
			     ...
			     conv_done == sc_logic(0) and zero_pad_done == sc_logic(1))
		{
			...
			//--------------------------------------------------------------------
			//В РАЗВЁРНУТЫХ ЦИКЛАХ ИСПРАВИТЬ k на нужный коэффициент в cout!!!
			//--------------------------------------------------------------------
			
			for (int i = 0; i < M3_param; i++) {//0   (высота/кол-во строк) выходного изображения
				for (int j = 0; j < N3_param; j++) {//(ширина/кол-во столбцов) выходного изображения
					for (int c = 0; c < C1_param; c++){//количество входных изображений
						for (int m = 0; m < M1_param; m++) {//(высота/кол-во строк) кернела
							for (int n = 0; n < N1_param; n++) {//(ширина/кол-во столбцов) кернела
								result[i][j][0] += 
								kernel_in[m][n][c][0] * image_in_padded[i + m][j + n][c];/**/
								if(verbose==1){
									cout<<std::scientific<<"result["<<i<<"]["<<j<<"][0]+=kernel_in["<<m<<"]["<<n<<"]["<<c
									<<"][0]*image_in["<<i<<"+"<<m<<"]["<<j<<"+"<<n<<"]["<<c<<"] | ";
									cout<<"result["<<i<<"]["<<j<<"][0]+="<<kernel_in[m][n][c][0]<<"*"
									<< image_in_padded[i + m][j + n][c]<<"="<<result[i][j][0]<<"\n";/**/
								}
								next_trigger();
							}		
						}
					...
					if(verbose==1){
						//cout<<"result_unbiased["<<i<<"]["<<j<<"][0]="<<result[i][j][0]<<"\n";/**/
					}
					result[i][j][0] += biases_in[0];
					if(verbose==1){
						cout<<"result_biased["<<i<<"]["<<j<<"][0]="<<result[i][j][0]<<"\n";/**/
					}
				}
			}
			for (int i = 0; i < M3_param; i++) {//1   (высота/кол-во строк) выходного изображения
				for (int j = 0; j < N3_param; j++) {//(ширина/кол-во столбцов) выходного изображения
					for (int c = 0; c < C1_param; c++){//количество входных изображений
						for (int m = 0; m < M1_param; m++) {//(высота/кол-во строк) кернела
							for (int n = 0; n < N1_param; n++) {//(ширина/кол-во столбцов) кернела
								result[i][j][1] += 
								kernel_in[m][n][c][1] * image_in_padded[i + m][j + n][c];/**/
								if(verbose==1){
									cout<<std::scientific<<"result["<<i<<"]["<<j<<"][1]+=kernel_in["<<m<<"]["<<n<<"]["<<c
									<<"][1]*image_in["<<i<<"+"<<m<<"]["<<j<<"+"<<n<<"]["<<c<<"] | ";
									cout<<"result["<<i<<"]["<<j<<"][1]+="<<kernel_in[m][n][c][1]<<"*"
									<< image_in_padded[i + m][j + n][c]<<"="<<result[i][j][1]<<"\n";/**/
								}
								next_trigger();
							}		
						}
					...
					if(verbose==1){
						//cout<<"result_unbiased["<<i<<"]["<<j<<"][1]="<<result[i][j][1]<<"\n";/**/
					}
					result[i][j][0] += biases_in[0];
					if(verbose==1){
						cout<<"result_biased["<<i<<"]["<<j<<"][1]="<<result[i][j][1]<<"\n";/**/
					}
				}
			}
			for (int i = 0; i < M3_param; i++) {//2   (высота/кол-во строк) выходного изображения
				for (int j = 0; j < N3_param; j++) {//(ширина/кол-во столбцов) выходного изображения
					for (int c = 0; c < C1_param; c++){//количество входных изображений
						for (int m = 0; m < M1_param; m++) {//(высота/кол-во строк) кернела
							for (int n = 0; n < N1_param; n++) {//(ширина/кол-во столбцов) кернела
								result[i][j][2] += 
								kernel_in[m][n][c][2] * image_in_padded[i + m][j + n][c];/**/
								if(verbose==1){
									cout<<std::scientific<<"result["<<i<<"]["<<j<<"][2]+=kernel_in["<<m<<"]["<<n<<"]["
									<<c<<"][2]*image_in["<<i<<"+"<<m<<"]["<<j<<"+"<<n<<"]["<<c<<"] | ";
									cout<<"result["<<i<<"]["<<j<<"][2]+="<<kernel_in[m][n][c][2]<<"*"
									<< image_in_padded[i + m][j + n][c]<<"="<<result[i][j][2]<<"\n";/**/
								}
								next_trigger();
							}		
						}
					...
					if(verbose==1){
						cout<<"result_unbiased["<<i<<"]["<<j<<"][2]="<<result[i][j][2]<<"\n";/**/
					}
					result[i][j][2] += biases_in[2];
					if(verbose==1){
						cout<<"result_biased["<<i<<"]["<<j<<"][2]="<<result[i][j][2]<<"\n";/**/
					}
				}
			}
			for (int i = 0; i < M3_param; i++) {//3   (высота/кол-во строк) выходного изображения
				for (int j = 0; j < N3_param; j++) {//(ширина/кол-во столбцов) выходного изображения
					for (int c = 0; c < C1_param; c++){//количество входных изображений
						for (int m = 0; m < M1_param; m++) {//(высота/кол-во строк) кернела
							for (int n = 0; n < N1_param; n++) {//(ширина/кол-во столбцов) кернела
								result[i][j][3] += 
								kernel_in[m][n][c][3] * image_in_padded[i + m][j + n][c];/**/
								if(verbose==1){
									cout<<std::scientific<<"result["<<i<<"]["<<j<<"][3]+=kernel_in["<<m<<"]["<<n<<"]["
									<<c<<"][3]*image_in["<<i<<"+"<<m<<"]["<<j<<"+"<<n<<"]["<<c<<"] | ";
									cout<<"result["<<i<<"]["<<j<<"][3]+="<<kernel_in[m][n][c][3]<<"*"
									<< image_in_padded[i + m][j + n][c]<<"="<<result[i][j][3]<<"\n";/**/
								}
								next_trigger();
							}		
						}
					...
					if(verbose==1){
						cout<<"result_unbiased["<<i<<"]["<<j<<"][3]="<<result[i][j][3]<<"\n";/**/
					}
					result[i][j][3] += biases_in[3];
					if(verbose==1){
						cout<<"result_biased["<<i<<"]["<<j<<"][3]="<<result[i][j][3]<<"\n";/**/
					}
